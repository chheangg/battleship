...
it('Path #1.1: Create main object horizontally', () => {
  expect(new Ship(Ships.battleship, 'horizontal', [0, 0]))
    .toMatchObject({
      length: 4,
      damage: [],
      axis: 'horizontal',
...
it('Path #1.2: Create main object vertically', () => {
  expect(new Ship(Ships.battleship, 'vertical', [0, 0]))
    .toMatchObject({
      length: 4,
      damage: [],
      axis: 'vertical',
...
it('Path #1.3: Create a small object', () => {
  expect(new Ship(Ships.patrol, 'horizontal', [0, 0]))
    .toMatchObject({
      length: 2,
      damage: [],
      axis: 'horizontal',
...
it('Path #2.1: Damage an object horziontally', () => {
  const prop = new Ship(Ships.battleship, 'horizontal', [0, 0]);
  prop.hit([0, 1]);
...
it('Path #2.2: Damage an object vertically', () => {
  const prop = new Ship(Ships.battleship, 'vertical', [0, 0]);
  prop.hit([1, 0]);
...
it('Path #2.3: Hit is out of range, horizontal object is fine', () => {
  const prop = new Ship(Ships.submarine, 'horizontal', [0, 0]);
  prop.hit([0, 4]);
...
it('Path #3.2: Sink a vertical boat', () => {
  const prop = new Ship(Ships.submarine, 'vertical', [0, 0]);
  prop.hit([0, 0]);
  prop.hit([1, 0]);
  prop.hit([2, 0]);
  expect(prop.isSunk()).toBe(true);
...
it('Path #3.4: single boat sinked', () => {
  const prop = new Ship(Ships.patrol, 'horizontal', [0, 0]);
  prop.hit([0, 0]);
  prop.hit([0, 1]);
  expect(prop.isSunk()).toBe(true);
});
