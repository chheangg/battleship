...
it('Path #1.1: Create main object horizontally', () => {
  ...
  expect(new Ship(ShipType.Battleship, player, Direction.PositiveX, new Coordinate(0, 0)))
    .toMatchObject({
      length: 4,
      damage: [],
...
it('Path #1.2: Create main object vertically', () => {
  const player = createTestPlayer();
  expect(new Ship(ShipType.Battleship, player, Direction.PositiveY, new Coordinate(0, 0)))
    .toMatchObject({
      length: 4,
      damage: [],
...
it('Path #1.3: Create a small object', () => {
  const player = createTestPlayer();
  expect(new Ship(ShipType.Patrol, player, Direction.PositiveX, new Coordinate(0, 0)))
    .toMatchObject({
      length: 2,
      damage: [],
      dir: Direction.PositiveX,
...
it('Path #2.1: Damage an object horziontally', () => {
  const player = createTestPlayer();
  const prop = new Ship(ShipType.Battleship, player, Direction.PositiveX, new Coordinate(0, 0));
...
it('Path #2.2: Damage an object vertically', () => {
  const player = createTestPlayer();
  const prop = new Ship(ShipType.Battleship, player, Direction.PositiveY, new Coordinate(0, 0));
...
it('Path #2.3: Hit is out of range, horizontal object is fine', () => {
  const player = createTestPlayer();
  const prop = new Ship(ShipType.Destroyer, player, Direction.PositiveX, new Coordinate(0, 0));
...
it('Path #3.2: Sink a vertical boat', () => {
  const player = createTestPlayer();
  const prop = new Ship(ShipType.Destroyer, player, Direction.PositiveY, new Coordinate(0, 0));
  prop.hit(new Coordinate(0, 0));
  prop.hit(new Coordinate(0, 1));
  prop.hit(new Coordinate(0, 2));
  expect(prop.isSunk()).toBe(true);
...
it('Path #3.4: single boat sinked', () => {
  const player = createTestPlayer();
  const prop = new Ship(ShipType.Patrol, player, Direction.PositiveX, new Coordinate(0, 0));
  prop.hit(new Coordinate(0, 0));
  prop.hit(new Coordinate(1, 0));
  expect(prop.isSunk()).toBe(true);
});
