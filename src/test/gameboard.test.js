...
it("Path #1.2 Placing two ship", () => {
  ...
  expect(board.list[1]).toMatchObject({
    length: 2,
    damage: [],
    dir: Direction.PositiveX,
    position: [new Coordinate(0, 1), new Coordinate(1, 1)],
  });
});
...
it("Path #3.1 Check hits", () => {
  ...
  expect(board.hits[0]).toMatchObject(new Coordinate(0, 0));
});
...
it("Path #3.2 Check misses", () => {
  ...
  expect(board.misses[0]).toMatchObject(new Coordinate(1, 0));
});
